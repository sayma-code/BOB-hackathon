"""
Analyze Bangladesh Export Data
Visualize export trends by product categories from 2000 onwards
"""
...
def load_data():
    ...
    # Load raw data
    raw_df = pd.read_csv('data/raw/bangladesh_export_data.csv')
    
    # Load pivot data
    pivot_df = pd.read_csv('data/processed/bangladesh_export_pivot.csv')
    
    # Load category data
    categories_df = pd.read_csv('data/processed/bangladesh_export_categories.csv')
...
def analyze_total_exports(pivot_df):
    """Analyze total export trends"""
    print("\n" + "="*80)
    print("TOTAL EXPORT ANALYSIS")
    print("="*80)
    ...
    # Get total exports column
    total_col = 'Total Merchandise Exports (current US$)'
    if total_col in pivot_df.columns:
        total_exports = pivot_df[['Year', total_col]].dropna()
        
        print(f"\nTotal Merchandise Exports (2000-2024):")
        print(f"  First year (2000): ${total_exports[total_col].iloc[0]:,.0f}")
        print(f"  Latest year: ${total_exports[total_col].iloc[-1]:,.0f}")
        print(f"  Growth: {((total_exports[total_col].iloc[-1] / total_exports[total_col].iloc[0]) - 1) * 100:.1f}%")
        print(f"  Average annual exports: ${total_exports[total_col].mean():,.0f}")
        
        return total_exports
    
    return None

def analyze_export_categories(categories_df):
    """Analyze export by product categories"""
    print("\n" + "="*80)
    print("EXPORT BY PRODUCT CATEGORY")
    print("="*80)
    ...
    print(f"\nExport Breakdown for {latest_year}:")
    ...
    print(f"\nTotal: ${latest_data['Total_Exports_USD'].iloc[0]:,.0f}")
...
def create_visualizations(pivot_df, categories_df):
    ...
    # 1. Total Exports Trend
    ax1 = plt.subplot(2, 3, 1)
    total_col = 'Total Merchandise Exports (current US$)'
    if total_col in pivot_df.columns:
        data = pivot_df[['Year', total_col]].dropna()
        ax1.plot(data['Year'], data[total_col] / 1e9, marker='o', linewidth=2, markersize=6, color='green')
        ax1.set_title('Total Merchandise Exports (2000-2024)', fontsize=14, fontweight='bold')
        ax1.set_xlabel('Year', fontsize=12)
        ax1.set_ylabel('Exports (Billion USD)', fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.tick_params(axis='x', rotation=45)
    
    # 2. Export Categories Over Time (Stacked Area)
    ...
    ax2.stackplot(category_pivot.index, 
                  *[category_pivot[col] for col in category_pivot.columns],
                  ...
                  alpha=0.8)
    ax2.set_title('Export Categories Over Time (Stacked)', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Year', fontsize=12)
    ax2.set_ylabel('Exports (Billion USD)', fontsize=12)
    ...
    colors = plt.cm.Paired(np.linspace(0, 1, len(latest_data)))
    ...
    ax3.set_title(f'Export Categories Breakdown ({latest_year})', fontsize=14, fontweight='bold')
    for text in texts:
        text.set_fontsize(9)
    ...
    ax4.set_title('Export Category Trends', fontsize=14, fontweight='bold')
    ax4.set_xlabel('Year', fontsize=12)
    ax4.set_ylabel('Exports (Billion USD)', fontsize=12)
    ...
    if total_col in pivot_df.columns:
        ...
        ax5.set_title('Year-over-Year Export Growth Rate', fontsize=14, fontweight='bold')
    ...
    for category in percentage_pivot.columns:
        ax6.plot(percentage_pivot.index, percentage_pivot[category], 
                marker='o', label=category, linewidth=2, markersize=4)
    ax6.set_title('Export Category Percentages Over Time', fontsize=14, fontweight='bold')
    ax6.set_xlabel('Year', fontsize=12)
    ax6.set_ylabel('Percentage of Total Exports (%)', fontsize=12)
    ...
    # Save figure
    output_dir = Path('analysis/exports')
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / 'export_analysis.png'
...
def generate_report(pivot_df, categories_df):
    ...
    report = []
    report.append("# Bangladesh Export Analysis Report (2000-2024)")
    report.append("\n## Executive Summary")
    report.append("\nThis report analyzes Bangladesh's export patterns from 2000 to 2024, ")
    report.append("categorizing products and tracking revenue trends over time.")
    
    # Total exports analysis
    report.append("\n## 1. Total Export Trends")
    total_col = 'Total Merchandise Exports (current US$)'
    if total_col in pivot_df.columns:
        data = pivot_df[['Year', total_col]].dropna()
        ...
        report.append(f"- **Average Annual Exports**: ${data[total_col].mean():,.0f}")
        
        # Year-by-year breakdown
        report.append("\n### Year-by-Year Export Values")
        report.append("\n| Year | Total Exports (USD) | Growth Rate (%) |")
        report.append("|------|---------------------|-----------------|")
    ...
    report.append(f"\n**Total Exports**: ${latest_data['Total_Exports_USD'].iloc[0]:,.0f}")
    ...
    # Largest category
    report.append(f"\n- **Largest Export Category ({latest_year})**: {latest_data.iloc[0]['Category']} ")
    report.append(f"(${latest_data.iloc[0]['Value_USD']:,.0f}, {latest_data.iloc[0]['Percentage']:.1f}%)")
    
    report.append("\n## 4. Visualizations")
    report.append("\n![Export Analysis](figures/export_analysis.png)")
    
    report.append("\n## 5. Data Sources")
    report.append("\n- World Bank Open Data API")
    report.append("- Indicators: Merchandise exports by product category")
    ...
    report_file = output_dir / 'bangladesh_export_report.md'
    ...
    categories_df.to_csv(output_dir / 'export_categories_detailed.csv', index=False)
    print(f"✓ Detailed category data saved to: {output_dir / 'export_categories_detailed.csv'}")

if __name__ == "__main__":
    print("="*80)
    print("BANGLADESH EXPORT DATA ANALYSIS")
    print("="*80)
    
    # Load data
    raw_df, pivot_df, categories_df = load_data()
    
    # Analyze total exports
    total_exports = analyze_total_exports(pivot_df)
    
    # Analyze categories
    latest_data = analyze_export_categories(categories_df)
    ...
    print("="*80)
    print("\nOutput files:")
    print("  - analysis/exports/bangladesh_export_report.md")
    print("  - analysis/exports/figures/export_analysis.png")
    print("  - analysis/exports/export_categories_detailed.csv")
